...
RULE_INT(Character, CorpseDecayTimeMS, 604800000, "Time after which the corpse decays (milliseconds)")
RULE_INT( Character, EmptyCorpseDecayTimeMS, 10800000, "Time after which an empty corpse decays (milliseconds")
RULE_INT(Character, CorpseResTimeMS, 10800000, "Time after which the corpse can no longer be resurrected (milliseconds)")
RULE_INT( Character, DuelCorpseResTimeMS, 600000, "Time before cant res corpse after a duel (milliseconds)")
...
RULE_REAL(Character, DragCorpseDistance, 5000, "If a player is using /corpsedrag and moving, the corpse will not move until the player exceeds this distance")
...
RULE_REAL(Character, GroupExpMultiplier, 1.0, "The experience in a group is multiplied by this value in addition to the group multiplier. The group multiplier is: 2 members=x 1.2, 3=x1.4, 4=x1.6, 5=x1.8, 6=x2.16")
...
RULE_BOOL(Character, ManaOnLevel, false, "Setting whether a player should have full mana when leveling")
...
RULE_BOOL(Character, EnableXTargetting, false, "Enable Extended Targeting Window, for users with UF and later clients")
...
RULE_BOOL(Character, NoSkillsOnHorse, true, "Enabling this will prevent Bind Wound and Foraging while on a mount")
RULE_BOOL(Character, UseNoJunkFishing, false, "Disregards junk items when fishing")
RULE_BOOL(Character, SoftDeletes, false, "When characters are deleted in character select, they are only soft deleted (PEQ SYSTEM NOT WFH)")
...
RULE_BOOL(Character, SkillUpFromItems, false, "Allow Skill ups from clickable items")
...
RULE_BOOL(Guild, GuildTributeDonationsEnabled, true, "Enabled/Disables Guild Tribute. Default is True.")
RULE_CATEGORY_END()
...
RULE_INT(GM, MinStatusToBypassLockedServer, 80, "Players >= this status can log in to the server even when it is locked")
RULE_INT(GM, MinStatusToBypassCheckSumVerification, 0, "Players >= this status can bypass the eqgame.exe and spells_us.txt checksum verification")
RULE_CATEGORY_END()
...
RULE_CATEGORY(Zone)
RULE_REAL(Zone, GroupEXPRange, 500, "Range to get XP when in group")
...
RULE_REAL(Zone, HotZoneBonus, 0.5, "Value which is added to the experience multiplier. This also applies to AA experience.")
...
RULE_INT( AlKabor, MaxEntitiesCharmTanks, 4, "If AllowCharmPetRaidTanks is false, this is the max number of entities on an NPC's hate list before the NPC will ignore charmed pets.  April 2003 patch set this to 4 on Live.")
RULE_CATEGORY_END()
...
RULE_REAL(Combat, AERampageMaxDistance, 82, "Max AERampage range (% of max combat distance)")
...
RULE_BOOL(Combat, ClassicNPCBackstab, true, "True disables NPC facestab - NPC get normal attack if not behind")
...
RULE_BOOL(Combat, SummonMeleeRange, false, "Enable or disable summoning of a player when already in melee range of the summoner.")
...
RULE_INT(Aggro, GrayAggroLevel, 19, "Gray Mobs will aggro when over this level")
...
RULE_BOOL(Aggro, PetsTriggerAggro, false, "Can pets Trigger Aggro")
...
RULE_BOOL(Chat, ChannelsIgnoreNameFilter, true, "Ignore name filtering when creating new chat channels")
...
RULE_CATEGORY(AA)
RULE_INT(AA, ExpPerPoint, 23976496, "Amount of experience per AA. Is the same as the amount of experience to go from level 51 to level 52")
...
RULE_INT(AA, UnusedAAPointCap, 30, "Cap for Unused AA Points.  Default: -1.  NOTE: DO NOT LOWER THIS WITHOUT KNOWING WHAT YOU ARE DOING. MAY RESULT IN PLAYERS LOSING AAs.")
RULE_CATEGORY_END()
...
RULE_BOOL(QueryServ, PlayerLogGuildTributeItemDonations, false, "Log player donations of guild tribute items.")
RULE_BOOL(QueryServ, PlayerLogGuildTributePlatDonations, false, "Log player donations of guild tribute plat.")
...
RULE_BOOL(Analytics, CrashReporting, false, "Automatic crash reporting analytics for EQEmu Server developers")
RULE_CATEGORY_END()
