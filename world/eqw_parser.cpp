...
EQWParser::EQWParser() {
	...
	_empty_sv = newSV(0);
	if (!my_perl) {
		Log(Logs::Detail, Logs::WorldServer, "Error: perl_alloc failed!");
	}
	else {
		DoInit();
	}
...
//this ties us for all packages
	...
	Log(Logs::Detail, Logs::WorldServer, "Loading worldui perl plugins.");
	std::string err;
	if(!eval_file("world", "worldui.pl", err)) {
		Log(Logs::Detail, Logs::WorldServer, "Warning - world.pl: %s", err.c_str());
	}
