...
std::string ClientList::GetWhoAll() {
	...
	while (iterator.MoreElements()) {
		...
		if (cle->Online() < CLE_Status::Zoning ||
			x > 20) {
			iterator.Advance();
			continue;
		}
		...
		reply.append(
				StringFormat(" %i %s ] %s", cle->level(), GetClassIDName(cle->class_(), cle->level()), cle->name()));
		reply.append(StringFormat(" %s zone: %s", GetRaceIDName(cle->race()), database.GetZoneName(cle->zone())));

		if (guild_mgr.GuildExists(cle->GuildID()))
			reply.append(StringFormat(" <%s>", guild_mgr.GetGuildName(cle->GuildID())));
