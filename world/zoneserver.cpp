    ...
    switch (opcode) {
        ...
        case ServerOP_SetConnectInfo: {
            ...
            if (!sci->port) {
                ...
                SendPacket(&p);
                Log(Logs::Detail, Logs::WorldServer, "Auto zone port configuration. Telling zone to use port %d",
                    client_port);
            } else {
        ...
        case ServerOP_SetLaunchName: {
            ...
            Log(Logs::Detail, Logs::WorldServer, "Zone started with name %s by launcher %s", launched_name.c_str(),
                launcher_name.c_str());
            break;
        }
        ...
        case ServerOP_ClientList: {
            if (pack->size != sizeof(ServerClientList_Struct)) {
                Log(Logs::Detail, Logs::WorldServer, "Wrong size on ServerOP_ClientList. Got: %d, Expected: %d",
        ...
        case ServerOP_ClientListKA: {
            ...
            if (pack->size < 4 || pack->size != 4 + (4 * sclka->numupdates)) {
                Log(Logs::Detail, Logs::WorldServer, "Wrong size on ServerOP_ClientListKA. Got: %d, Expected: %d",
        ...
        case ServerOP_GMGoto: {
            if (pack->size != sizeof(ServerGMGoto_Struct)) {
                Log(Logs::Detail, Logs::WorldServer, "Wrong size on ServerOP_GMGoto. Got: %d, Expected: %d",
        ...
        case ServerOP_Lock: {
            if (pack->size != sizeof(ServerLock_Struct)) {
                Log(Logs::Detail, Logs::WorldServer, "Wrong size on ServerOP_Lock. Got: %d, Expected: %d", pack->size,
        ...
        case ServerOP_Motd: {
            if (pack->size != sizeof(ServerMotd_Struct)) {
                Log(Logs::Detail, Logs::WorldServer, "Wrong size on ServerOP_Motd. Got: %d, Expected: %d", pack->size,
        ...
        case ServerOP_Uptime: {
            if (pack->size != sizeof(ServerUptime_Struct)) {
                Log(Logs::Detail, Logs::WorldServer, "Wrong size on ServerOP_Uptime. Got: %d, Expected: %d",
        ...
        case ServerOP_LockZone: {
            if (pack->size < sizeof(ServerLockZone_Struct)) {
                Log(Logs::Detail, Logs::WorldServer, "Wrong size on ServerOP_LockZone. Got: %d, Expected: %d",
        ...
        default: {
            Log(Logs::Detail, Logs::WorldServer, "Unknown ServerOPcode from zone 0x%04x, size %d", pack->opcode,
                pack->size);
