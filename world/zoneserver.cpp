    ...
    switch (opcode) {
                    ...
                    if (console) {
                        ...
                        if (((!con) || (!con->SendChannelMessage(scm, [&scm]() {
                            auto pack = new ServerPacket(ServerOP_ChannelMessage,
                                                         sizeof(ServerChannelMessage_Struct) + strlen(scm->message) +
                                                         1);
                            ...
                            zoneserver_list.SendEmoteMessage(scm->from, 0, 0, 0, "%s is not online at this time.",
                                                             scm->to);
                        }
                    }
                    break;
                ...
                if (cle == 0 || cle->Online() < CLE_Status_Zoning ||
                    ...
                    if (!scm->noreply) {
                        ...
                        if (cle->TellQueueFull()) {
                            ...
                            memset(temp, 0,
                                   struct_size); // just in case, was seeing some corrupt messages, but it shouldn't happen
                    ...
                    if (!scm->noreply)
                        zoneserver_list.SendEmoteMessage(scm->from, 0, 0, 0,
                                                         "You told %s, '%s is not contactable at this time'", scm->to,
                                                         scm->to);
                } else
        ...
        case ServerOP_SetConnectInfo: {
            ...
            if (!sci->port) {
                ...
                SendPacket(&p);
                Log(Logs::Detail, Logs::World_Server, "Auto zone port configuration. Telling zone to use port %d",
                    client_port);
            } else {
        ...
        case ServerOP_SetLaunchName: {
            ...
            Log(Logs::Detail, Logs::World_Server, "Zone started with name %s by launcher %s", launched_name.c_str(),
                launcher_name.c_str());
            break;
        }
        ...
        case ServerOP_ZoneShutdown: {
            ...
            if (s->ZoneServerID != 0)
                ...
                zoneserver_list.SendEmoteMessage(s->adminname, 0, 0, 0,
        ...
        case ServerOP_ZoneBootup: {
            ServerZoneStateChange_struct *s = (ServerZoneStateChange_struct *) pack->pBuffer;
            zoneserver_list.SOPZoneBootup(s->adminname, s->ZoneServerID, database.GetZoneName(s->zoneid),
                                          s->makestatic);
            break;
        }
        ...
        case ServerOP_ZoneToZoneRequest: {
            ...
            if (GetZoneID() == ztz->current_zone_id && GetInstanceID() == ztz->current_instance_id) {
                ...
                if (ztz->admin < 80 && ztz->ignorerestrictions < 2 &&
        ...
        case ServerOP_ClientList: {
            if (pack->size != sizeof(ServerClientList_Struct)) {
                Log(Logs::Detail, Logs::World_Server, "Wrong size on ServerOP_ClientList. Got: %d, Expected: %d",
        ...
        case ServerOP_ClientListKA: {
            ...
            if (pack->size < 4 || pack->size != 4 + (4 * sclka->numupdates)) {
                Log(Logs::Detail, Logs::World_Server, "Wrong size on ServerOP_ClientListKA. Got: %d, Expected: %d",
                    pack->size, (4 + (4 * sclka->numupdates)));
                break;
            }
        ...
        case ServerOP_GMGoto: {
            if (pack->size != sizeof(ServerGMGoto_Struct)) {
                Log(Logs::Detail, Logs::World_Server, "Wrong size on ServerOP_GMGoto. Got: %d, Expected: %d",
            ...
            if (cle != 0) {
                if (cle->Server() == 0)
                    this->SendEmoteMessage(gmg->myname, 0, 0, 13, "Error: Cannot identify %s's zoneserver.",
                                           gmg->gotoname);
        ...
        case ServerOP_Lock: {
            if (pack->size != sizeof(ServerLock_Struct)) {
                Log(Logs::Detail, Logs::World_Server, "Wrong size on ServerOP_Lock. Got: %d, Expected: %d", pack->size,
                    sizeof(ServerLock_Struct));
                break;
            }
        ...
        case ServerOP_Motd: {
            if (pack->size != sizeof(ServerMotd_Struct)) {
                Log(Logs::Detail, Logs::World_Server, "Wrong size on ServerOP_Motd. Got: %d, Expected: %d", pack->size,
                    sizeof(ServerMotd_Struct));
                break;
            }
        ...
        case ServerOP_Uptime: {
            if (pack->size != sizeof(ServerUptime_Struct)) {
                Log(Logs::Detail, Logs::World_Server, "Wrong size on ServerOP_Uptime. Got: %d, Expected: %d",
                    pack->size, sizeof(ServerUptime_Struct));
                break;
            }
        ...
        case ServerOP_SetWorldTime: {
            ...
            Log(Logs::Detail, Logs::World_Server, "New time = %d-%d-%d %d:%d (%d)\n", newtime->start_eqtime.year,
                newtime->start_eqtime.month, (int) newtime->start_eqtime.day, (int) newtime->start_eqtime.hour,
                (int) newtime->start_eqtime.minute, (int) newtime->start_realtime);
            database.SaveTime((int) newtime->start_eqtime.minute, (int) newtime->start_eqtime.hour,
                              (int) newtime->start_eqtime.day, newtime->start_eqtime.month, newtime->start_eqtime.year);
            zoneserver_list.SendTimeSync();
            break;
        }
        case ServerOP_IPLookup: {
            if (pack->size < sizeof(ServerGenericWorldQuery_Struct)) {
                Log(Logs::Detail, Logs::World_Server,
                    "Wrong size on ServerOP_IPLookup. Got: %d, Expected (at least): %d", pack->size,
        ...
        case ServerOP_LockZone: {
            if (pack->size < sizeof(ServerLockZone_Struct)) {
                Log(Logs::Detail, Logs::World_Server, "Wrong size on ServerOP_LockZone. Got: %d, Expected: %d",
                    pack->size, sizeof(ServerLockZone_Struct));
                break;
            }
            ...
            switch (s->op) {
                ...
                case 1:
                    if (zoneserver_list.SetLockedZone(s->zoneID, true))
                        zoneserver_list.SendEmoteMessage(0, 0, 80, 15, "Zone locked: %s",
                                                         database.GetZoneName(s->zoneID));
                    else
                        this->SendEmoteMessageRaw(s->adminname, 0, 0, 0, "Failed to change lock");
                    break;
                case 2:
                    if (zoneserver_list.SetLockedZone(s->zoneID, false))
                        zoneserver_list.SendEmoteMessage(0, 0, 80, 15, "Zone unlocked: %s",
                                                         database.GetZoneName(s->zoneID));
                    else
                        this->SendEmoteMessageRaw(s->adminname, 0, 0, 0, "Failed to change lock");
                    break;
                ...
                if (cle->instance() != 0) {
                    ...
                    if (zs) {
                        ...
                        if (zs) {
                            ...
                            Log(Logs::Detail, Logs::World_Server,
                                "Unable to locate zone record for instance id %u in zoneserver list for ServerOP_Consent_Response operation.",
                                s->instance_id);
                        }
                        safe_delete(pack);
                    }
                    ...
                    if (zs) {
                        ...
                        if (zs) {
                            ...
                            Log(Logs::Detail, Logs::World_Server,
                                "Unable to locate zone record for zone id %u in zoneserver list for ServerOP_Consent_Response operation.",
                                s->zone_id);
                        }
                        safe_delete(pack);
                    }
        ...
        default: {
            Log(Logs::Detail, Logs::World_Server, "Unknown ServerOPcode from zone 0x%04x, size %d", pack->opcode,
                pack->size);
