			...
			for (auto row_d = results_bind.begin(); row_d != results_bind.end(); ++row_d) {
				...
				if (has_home == 0) {
					std::string query = StringFormat(
							...
							character_id, pp.binds[4].zoneId, 0, pp.binds[4].x, pp.binds[4].y, pp.binds[4].z,
							pp.binds[4].heading, 4);
...
bool WorldDatabase::GetStartZone(PlayerProfile_Struct* in_pp, CharCreate_Struct* in_cc,bool isTitanium)
{
	...
	std::string query = StringFormat("SELECT x, y, z, heading, start_zone, bind_id, bind_x, bind_y, bind_z FROM start_zones WHERE zone_id = %u "
		"AND player_class = %u AND player_deity = %u AND player_race = %u",
