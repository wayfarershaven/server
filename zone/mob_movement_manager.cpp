...
void MobMovementManager::UpdatePath(Mob *who, float x, float y, float z, MobMovementMode mode) {
	...
	if (who->IsBoat()) {
		...
		PushFlyTo(ent.second, x, y, z, mode);
		PushStopMoving(ent.second);
	} else {
...
void MobMovementManager::PushFlyTo(MobMovementEntry &ent, float x, float y, float z, MobMovementMode mode)
{
	ent.Commands.push_back(std::unique_ptr<IMovementCommand>(new FlyToCommand(x, y, z, mode)));
}
