...
bool Map::LoadMMF(const std::string& map_file_name, bool force_mmf_overwrite)
{
	...
	if (!add_mmf_extension(mmf_file_name)) {
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s'", mmf_file_name.c_str());
		return false;
	}

	FILE *f = fopen(mmf_file_name.c_str(), "rb");
	if (!f) {
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - could not open file", mmf_file_name.c_str());
		return false;
	}
	...
	if (fread(&file_version, sizeof(uint32), 1, f) != 1) {
		fclose(f);
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - f@file_version", mmf_file_name.c_str());
		return false;
	}
	...
	if (fread(&rm_buffer_size, sizeof(uint32), 1, f) != 1) {
		fclose(f);
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - f@rm_buffer_size", mmf_file_name.c_str());
		return false;
	}
	...
	if (fread(&rm_buffer_crc32, sizeof(uint32), 1, f) != 1) {
		fclose(f);
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - f@rm_buffer_crc32", mmf_file_name.c_str());
		return false;
	}
	if (rm_buffer_crc32 != /*crc32_check*/ 0) {
		fclose(f);
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - bad rm_buffer checksum", mmf_file_name.c_str());
		return false;
	}
	...
	if (fread(&mmf_buffer_size, sizeof(uint32), 1, f) != 1) {
		fclose(f);
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - f@mmf_buffer_size", mmf_file_name.c_str());
		return false;
	}
	...
	if (fread(mmf_buffer.data(), mmf_buffer_size, 1, f) != 1) {
		fclose(f);
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - f@mmf_buffer", mmf_file_name.c_str());
		return false;
	}
	...
	if (!imp->rm) {
		delete imp;
		imp = nullptr;
		Log(Logs::General, Logs::ZoneServer, "Failed to load Map MMF file: '%s' - null RaycastMesh", mmf_file_name.c_str());
		return false;
	}
...
{
	if (!imp || !imp->rm) {
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file - No implementation (map_file_name: '%s')", map_file_name.c_str());
		return false;
	}
	...
	strip_map_extension(mmf_file_name);
	if (!add_mmf_extension(mmf_file_name)) {
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s'", mmf_file_name.c_str());
		return false;
	}
	...
	if (rm_buffer.empty()) {
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - empty RaycastMesh buffer", mmf_file_name.c_str());
		return false;
	}
	...
	if (!mmf_buffer_size) {
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - null MMF buffer size", mmf_file_name.c_str());
		return false;
	}
	
	f = fopen(mmf_file_name.c_str(), "wb");
	if (!f) {
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - could not open file", mmf_file_name.c_str());
		return false;
	}
	...
	if (fwrite(&file_version, sizeof(uint32), 1, f) != 1) {
		...
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - f@file_version", mmf_file_name.c_str());
		return false;
	}
	...
	if (fwrite(&rm_buffer_size, sizeof(uint32), 1, f) != 1) {
		fclose(f);
		std::remove(mmf_file_name.c_str());
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - f@rm_buffer_size", mmf_file_name.c_str());
		return false;
	}
	...
	if (fwrite(&rm_buffer_crc32, sizeof(uint32), 1, f) != 1) {
		fclose(f);
		std::remove(mmf_file_name.c_str());
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - f@rm_buffer_crc32", mmf_file_name.c_str());
		return false;
	}
	...
	if (fwrite(&mmf_buffer_size, sizeof(uint32), 1, f) != 1) {
		fclose(f);
		std::remove(mmf_file_name.c_str());
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - f@mmf_buffer_size", mmf_file_name.c_str());
		return false;
	}
	...
	if (fwrite(mmf_buffer.data(), mmf_buffer_size, 1, f) != 1) {
		fclose(f);
		std::remove(mmf_file_name.c_str());
		Log(Logs::General, Logs::ZoneServer, "Failed to save Map MMF file: '%s' - f@mmf_buffer", mmf_file_name.c_str());
		return false;
	}
