...
int Mob::GetBaseSkillDamage(EQEmu::skills::SkillType skill, Mob *target)
{
	...
	case EQEmu::skills::SkillTigerClaw: {
		if (IsClient()) {
			auto inst = CastToClient()->GetInv().GetItem(EQEmu::invslot::slotHands);
			if (inst)
	...
	case EQEmu::skills::SkillBash: {
		...
		if (IsClient()) {
			if (HasShieldEquiped())
				...
				inst = CastToClient()->GetInv().GetItem(EQEmu::invslot::slotShoulders);
		}
		if (inst)
	...
	if ((hate_top && hate_top->GetHPRatio() >= 20) || hate_top == nullptr || chance_bonus) {
		...
		if (Success) {
			if (hate_top && hate_top != this) {
				int newhate = (who->GetNPCHate(hate_top) - who->GetNPCHate(this)) + 101 + bonus_hate;
