...
int command_init(void)
{
	...
	if (
		...
		command_add("toggle", " - Toggle server options", 30, command_toggle) ||
...
void command_npcstats(Client *c, const Seperator *sep)
{
	if (c->GetTarget() == 0)
        ...
        c->Message(0, "- Spawn Group: %i  Grid: %i", target_npc->GetSpawnGroupId(), target_npc->GetGrid());
...
void command_stats(Client *c, const Seperator *sep)
{
	...
	c->Message(0, "Spawn Group: %i  Grid: %i", c->GetTarget()->CastToNPC()->GetSpawnGroupId(), c->GetTarget()->CastToNPC()->GetGrid());
...
void command_issue(Client *c, const Seperator *sep) {
	...
	std::string query = StringFormat("INSERT INTO issues"
									 ...
									 ((c->GetTarget() == nullptr || !c->GetTarget()->IsNPC()) ? 0 : c->GetTarget()->CastToNPC()->GetSpawnGroupId()),
									 itemid,
...
void command_corpse(Client *c, const Seperator *sep)
{
	...
	if (strcasecmp(sep->arg[1], "lock") == 0) {
        ...
        */
		...
		c->Message(0, "  summon - Summon target or name - Example #corpse summon mythsong OR #corpse summon with a target");
		c->Message(0, "  buried - #corpse buried list, #corpse buried summon");
		c->Message(0, "  backup - #corpse backup list, #corpse backup summon");
		c->Message(0, "  delete (Corpsenumber) - Delete a corpse permanently. Corpsenumber optional if no corpse target");
...
{	if (!c->GetTarget() || !c->GetTarget()->IsNPC()) {
	...
	if (strcasecmp(sep->arg[1], "setanimation") == 0) {
		...
		c->Message(15,"NPCID %u now has the animation set to %i on spawn with spawngroup %i",  npcTypeID, animation, c->GetTarget()->CastToNPC()->GetSpawnGroupId() );
...
void command_advnpcspawn(Client *c, const Seperator *sep)
{
	...
	if (strcasecmp(sep->arg[1], "setversion") == 0) {
		...
		c->Message(0, "Version change to %i was successful from SpawnGroupID %i",  version, c->GetTarget()->CastToNPC()->GetSpawnGroupId());
