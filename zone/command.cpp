	...
	else if (strcasecmp(sep->arg[1], "add") == 0)
	{
		...
		if (c->GetTarget()->IsNPC() && sep->IsNumber(2))
		{
			...
			if (database.GetItem(item))
			{
				...
				c->Message(Chat::White, "Added item(%i) to the %s's loot. isQuest: %s",  item, c->GetTarget()->GetName(), isQuest.c_str());
			}
			else
...
void command_issue(Client *c, const Seperator *sep) {
	...
	if (sep->arg[1] && strcasecmp(sep->arg[1], "delete") == 0) { //Delete an issue
		if (!sep->arg[2] || atoi(sep->arg[2]) == 0) {
			c->Message(Chat::White, "Invalid issue id. Format: #issue delete <number>");
			return;
		}
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Red, "Deleting an issue failed. The admins have been notified.");
			Log(Logs::General, Logs::Normal, "#issue creation failed for user %u: %s", c->CharacterID(), results.ErrorMessage().c_str());
			return;
		}
		c->Message(Chat::White, "You have deleted issue #%u.", issue_id);
		return;
	}
	...
	if (sep->arg[1] && strcasecmp(sep->arg[1], "list") == 0) { //List past issues
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Red, "Listing issues failed. The admins have been notified.");
			Log(Logs::General, Logs::Normal, "#issue list failed for user %u: %s", c->CharacterID(), results.ErrorMessage().c_str());
			return;
		}
		if (results.RowCount() == 0) {
			c->Message(Chat::White, "You have no pending issues.");
			return;
		}

		c->Message(Chat::White, "Your %u most recently updated issues:", results.RowCount());
		for (auto row = results.begin(); row != results.end(); ++row) {
			...
			c->Message(Chat::White, "#%u status: %s, details: %s [ %s ]", atoi(row[0]), status.c_str(), details.c_str(), c->CreateSayLink(deletecommand.c_str(), "delete").c_str());
		}
		return;
	}
	...
	if (!sep->arg[1] || (strlen(sep->arg[1]) == 0)) {
		...
		c->Message(Chat::White, "To report something to the GMs, you may target a mob or player and then:");
		c->Message(Chat::White, "/say #issue Your report message");
		return;
	}
	...
	std::string query = StringFormat("INSERT INTO issues"
									 ...
									 EQ::versions::ClientVersionName(c->ClientVersion())
	);
	auto results = database.QueryDatabase(query);
	if (!results.Success()) {
		c->Message(Chat::Red, "Creating an issue failed. The admins have been notified.");
		Log(Logs::General, Logs::Normal, "#issue creation failed for user %u: %s", c->CharacterID(), results.ErrorMessage().c_str());
		return;
	}
...
void command_corpse(Client *c, const Seperator *sep) {
	...
	// Locks a corpse, preventing anybody from looting it
	if (strcasecmp(sep->arg[1], "lock") == 0) {
		...
		// Unlocks a corpse, preventing normal looting
		...
		// Lists all PC corpses in zone with location
	else if (strcasecmp(sep->arg[1], "listPC") == 0) {
		entity_list.ListPlayerCorpses(c);
	}
		// Lists all NPC corpses in zone with location
	else if (strcasecmp(sep->arg[1], "listNPC") == 0) {
		entity_list.ListNPCCorpses(c);
	}
		// Depops a corpse with optional bury (1)
		...
		if (target == 0 || !target->IsPlayerCorpse())
			...
			if (!sep->arg[2][0] || atoi(sep->arg[2]) != 0) {
				c->Message(0, "Burying %s.", target->GetName());
	...
	} else if (strcasecmp(sep->arg[1], "summon") == 0) {
		Corpse *crps;
		const char *name;
		...
		if (strlen(sep->arg[2]) > 0) {
			...
			if (target == 0)
				c->Message(0, "Must have target if not specifying a name of corpse to summon.");
			else {
				name = target->GetName();
...
void command_showbonusstats(Client *c, const Seperator *sep)
{
	if (c->GetTarget() == 0)
		...
		if (bAll || (strcasecmp(sep->arg[1], "spell")==0)) {
			...
			c->Message(Chat::White, " Crit: %i%%", c->GetTarget()->GetSpellBonuses().CriticalHitChance);
			if (c->GetTarget()->GetClass() == PALADIN) {
				c->Message(Chat::White, "Slay Rate: %i%%m Slay Damage Mod: %i%%", c->GetTarget()->GetSpellBonuses().SlayUndead[0], c->GetTarget()->GetSpellBonuses().SlayUndead[1]);
			}
		}
		if (bAll || (strcasecmp(sep->arg[1], "aa") == 0)) {
			c->Message(Chat::White, " Ambidexterity: %i%% ", c->GetTarget()->GetAABonuses().Ambidexterity);
			if (c->GetTarget()->IsPet()) {
				c->Message(Chat::White, " PetFlurry: %i%% ", c->GetTarget()->GetAABonuses().PetFlurry);
				c->Message(Chat::White, " PC_Pet_Flurry: %i%% ", c->GetTarget()->GetAABonuses().PC_Pet_Flurry);
				c->Message(Chat::White, " PC_Pet_Rampage: %i%% ", c->GetTarget()->GetAABonuses().PC_Pet_Rampage);
				c->Message(Chat::White, " PetCriticalHit: %i%% ", c->GetTarget()->GetAABonuses().PetCriticalHit);
			}
		}
