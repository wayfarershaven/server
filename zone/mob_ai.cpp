...
void Client::AI_Stop() {
	Mob::AI_Stop();
	this->MessageString(13,PLAYER_REGAIN);
...
bool Mob::Flurry(ExtraAttackOptions *opts)
{
	...
	if (target) {
		if (!IsPet()) {
			entity_list.MessageCloseString(this, true, 200, Chat::NPCFlurry, NPC_FLURRY, GetCleanName(), target->GetCleanName());
		} else {
			entity_list.MessageCloseString(this, true, 200, Chat::PetFlurry, NPC_FLURRY, GetCleanName(), target->GetCleanName());
		}
