...
void Trap::Trigger(Mob* trigger)
{
	...
	switch (effect)
	{
		case trapTypeDebuff:
			if(message.empty())
			{
				entity_list.MessageClose(trigger,false,100,0,"%s triggers a trap!",trigger->GetName());
			}
			else
			{
				entity_list.MessageClose(trigger,false,100,0,"%s",message.c_str());
			}
			if(hiddenTrigger){
		...
		case trapTypeAlarm:
			if (message.empty())
			{
				entity_list.MessageClose(trigger,false,effectvalue,0,"A loud alarm rings out through the air...");
			}
			else
			{
				entity_list.MessageClose(trigger,false,effectvalue,0,"%s",message.c_str());
			}
		...
		case trapTypeMysticSpawn:
			if (message.empty())
			{
				entity_list.MessageClose(trigger,false,100,0,"The air shimmers...");
			}
			else
			{
				entity_list.MessageClose(trigger,false,100,0,"%s",message.c_str());
		...
		case trapTypeBanditSpawn:
			if (message.empty())
			{
				entity_list.MessageClose(trigger,false,100,0,"A bandit leaps out from behind a tree!");
			}
			else
			{
				entity_list.MessageClose(trigger,false,100,0,"%s",message.c_str());
	...
	for (auto it = client_list.begin(); it != client_list.end(); ++it) {
		...
		if ((diff.x * diff.x + diff.y * diff.y) <= maxdist
			...
			if (cur->trapid == 0 && !cur->GetGM() && (trap->chance == 0 || zone->random.Roll(trap->chance))) {
				Log(Logs::General, Logs::Traps,
					"%s is about to trigger trap %d of chance %d. diff: %0.2f maxdist: %0.2f zdiff: %0.2f maxzdiff: %0.2f",
					cur->GetName(), trap->trap_id, trap->chance, (diff.x * diff.x + diff.y * diff.y), maxdist, diff.z,
					trap->maxzdiff);
				return (cur);
			}
		} else {
	...
	while (it != trap_list.end()) {
		...
		if (cur->IsTrap()) {
			...
			client->Message(Chat::White,
							" Trap: (%d) found at %0.2f,%0.2f,%0.2f. Times Triggered: %d Is Active: %d Group: %d Message: %s",
							cur->trap_id, cur->m_Position.x, cur->m_Position.y, cur->m_Position.z, cur->times_triggered,
							isset, cur->group, cur->message.c_str());
			++count;
		}
		++it;
	}
