...
void NatsManager::GetCommandMessage(eqproto::CommandMessage* message, const char* reply) {
	...
	if (message->command().compare("npctypespawn") == 0) {
		...
		auto npc = new NPC(tmp, 0, position, GravityBehavior::Flying);
		if (!npc) {
			message->set_response_error(eqproto::ERR_Failed);
...
bool NatsManager::connect() {
	...
	std::string connection = StringFormat("nats://localhost:4222", zoneConfig->NATSHost.c_str(), zoneConfig->NATSPort);
