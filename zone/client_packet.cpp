...
void Client::Handle_OP_GroupInvite2(const EQApplicationPacket *app)
{
	...
	if (Invitee) {
		if (Invitee->IsClient()) {
			if (Invitee->HasGroup()) {
				MessageString(Chat::LightGray, TARGET_ALREADY_IN_GROUP, Invitee->GetCleanName());
				return;
			}
...
void Client::Handle_OP_GuildBank(const EQApplicationPacket *app)
{
	...
	case GuildBankWithdraw:
	{
		...
		{
			Message(Chat::White, "Unable to withdraw 0 quantity of %s", inst->GetItem()->Name);
		}
...
void Client::Handle_OP_GuildDemote(const EQApplicationPacket *app)
{
	...
	if (!IsInAGuild())
		...
		Message(Chat::White, "You dont have permission to invite.");
	else if (!worldserver.Connected())
		Message(Chat::White, "Error: World server disconnected");
	else {
		...
		if (!guild_mgr.GetCharInfo(demote->target, gci)) {
			Message(Chat::White, "Unable to find '%s'", demote->target);
			return;
		}
		...
		if (gci.rank < 1) {
			Message(Chat::White, "%s cannot be demoted any further!", demote->target);
			return;
		}
...
void Client::Handle_OP_GuildManageBanker(const EQApplicationPacket *app)
{
	...
	{
		Message(Chat::White, "Unable to find '%s'", gmb->member);
		return;
	}
...
void Client::Handle_OP_GuildPromote(const EQApplicationPacket *app)
{
	...
	if (!IsInAGuild())
		...
		if (!guild_mgr.GetCharInfo(promote->target, gci)) {
			Message(Chat::White, "Unable to find '%s'", promote->target);
			return;
		}
		if (gci.guild_id != GuildID()) {
			Message(Chat::White, "You aren't in the same guild, what do you think you are doing?");
		...
		{
			Message(Chat::White, "You cannot promote someone to be guild leader. You must use /guildleader.");
			return;
		}
...
void Client::Handle_OP_GuildPublicNote(const EQApplicationPacket *app)
{
	...
	if (!guild_mgr.GetCharInfo(gpn->target, gci)) {
		Message(Chat::White, "Unable to find '%s'", gpn->target);
		return;
	}
	if (gci.guild_id != GuildID()) {
		Message(Chat::White, "You aren't in the same guild, what do you think you are doing?");
...
void Client::Handle_OP_LFGCommand(const EQApplicationPacket *app)
{
	...
	case 1:
		...
		break;
	default:
		Message(Chat::White, "Error: unknown LFG value %i", lfg->value);
	}
...
void Client::Handle_OP_PetCommands(const EQApplicationPacket *app)
{
	...
	*/
	...
	case PET_BACKOFF: {
		if (mypet->IsFeared()) {
			break; //keeps pet running while feared
		}
...
void Client::Handle_OP_Sneak(const EQApplicationPacket *app) {
	...
	if (sneaking) {
		...
		if (!is_underwater) {
			...
			Message(Chat::White, "You cannot start sneaking underwater.");
			return;
		}
	}
...
void Client::Handle_OP_WhoAllRequest(const EQApplicationPacket *app)
{
	...
	Message(Chat::White, StringFormat("%s , %u , %u, %u, %u, %u, %u, %s, %u",
