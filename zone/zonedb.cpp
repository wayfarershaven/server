...
const NPCType* ZoneDatabase::LoadNPCTypesData(uint32 npc_type_id, bool bulk_load /*= false*/)
{
    ...
    for (auto row = results.begin(); row != results.end(); ++row) {
		...
		temp_npctype_data->skip_global_loot = atoi(row[102]) != 0;
		temp_npctype_data->rare_spawn = atoi(row[103]) != 0;
...
uint32 ZoneDatabase::SaveCharacterCorpse(uint32 charid, const char *charname, uint32 zoneid, uint16 instanceid, PlayerCorpse_Struct *dbpc, const glm::vec4 &position, uint32 guildid, bool backup)
{
	...
	/*
	// handle backup
	if (backup && (dbpc->itemcount != 0 || dbpc->copper != 0 || dbpc->silver != 0 || dbpc->gold != 0 || dbpc->plat != 0)) {
		query.insert(36, StringFormat("`id` = %u, ", last_insert_id));
		query.insert(30, "_backup");
		QueryDatabase(query);
		if (dbpc->itemcount != 0) {
			corpse_items_query.insert(36, "_backup");
			QueryDatabase(corpse_items_query);
		}
	}
	*/
...
bool ZoneDatabase::IsCorpseOwner(uint32 corpse_id, uint32 char_id, bool backup) {
	std::string query;
	if (backup)
		query = StringFormat("SELECT COUNT(*) FROM `character_corpses_backup` WHERE `id` = %d AND `charid` = %d", corpse_id, char_id);
	else
		query = StringFormat("SELECT COUNT(*) FROM `character_corpses` WHERE `id` = %d AND `charid` = %d", corpse_id, char_id);
	auto results = QueryDatabase(query);
	auto row = results.begin();
	if (atoi(row[0]) == 1)
		return true;
