...
{
    Log(Logs::Detail, Logs::Expeditions, "Updating replay lockout on join [%b] for expedition [%i]", add_on_join, expedition_id);
    ...
    if (!results.Success())
    {
        Log(Logs::General, Logs::Expeditions, "Failed to update expedition [%i] replay timer setting", expedition_id);
    }
}