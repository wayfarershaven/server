	...
	if ((GetLevel() != check_level) && !(check_level >= maxlevel)) {
		...
		if (level_increase)
		{
			...
			if (check_level == RuleI(Character, DeathItemLossLevel))
				MessageString(15, CORPSE_ITEM_LOST);

			if (check_level == RuleI(Character, DeathExpLossLevel))
				MessageString(15, CORPSE_EXP_LOST);
		}
		else
...
/*	if (admin>=100 && GetGM()) {
		...
		char val2[20]={0};
		char val3[20]={0};
		MessageString(Chat::Yellow, GM_GAINXP,ConvertArray(set_aaxp,val1),ConvertArray(set_exp,val2),ConvertArray(GetEXPForLevel(GetLevel()+1),val3)); //[GM] You have gained %1 AXP and %2 EXP (%3).
