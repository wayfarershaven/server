    ...
    switch (opcode) {
        ...
        case ServerOP_ChannelMessage: {
            ...
            if (scm->deliverto[0] == 0) {
                entity_list.ChannelMessageFromWorld(scm->from, scm->to, scm->chan_num, scm->guilddbid, scm->language,
                                                    scm->message);
            } else {
        ...
        case ServerOP_SpawnCondition: {
            ...
            zone->spawn_conditions.SetCondition(zone->GetShortName(), zone->GetInstanceID(), ssc->condition_id,
                                                ssc->value, true);
            break;
        }
        ...
        case ServerOP_ZoneToZoneRequest: {
            ...
            if (ztz->current_zone_id == zone->GetZoneID()
                ...
                switch (ztz->response) {
                    case -2: {
                        entity->CastToClient()->Message(13,
                                                        "You do not own the required locations to enter this zone.");
                        break;
                    }
                    case -1: {
                    ...
                    case 0: {
                        entity->CastToClient()->Message(13,
                                                        "All zone servers are taken at this time, please try again later.");
                        break;
                    }
                }
            } else {
            ...
            if (wars && wars->id != 0 && wars->id < 0xFFFFFFFF) {
                ...
                if (client) {
                    ...
                    Log(Logs::Detail, Logs::None,
                        "[CLIENT] id=%i, playerineqstring=%i, playersinzonestring=%i. Dumping WhoAllReturnStruct:",
                ...
                if (strcasecmp(sem->to, zone->GetShortName()) == 0)
                    ...
                    if (client != 0) {
                        ...
                        if (strstr(sem->message, "^") == 0)
                            ...
                            for (newmessage = strtok((char *) sem->message, "^");
                                 newmessage != nullptr; newmessage = strtok(nullptr, "^"))
        ...
        case ServerOP_ZoneShutdown: {
            if (pack->size != sizeof(ServerZoneStateChange_struct)) {
                std::cout << "Wrong size on ServerOP_ZoneShutdown. Got: " << pack->size << ", Expected: "
                          << sizeof(ServerZoneStateChange_struct) << std::endl;
                break;
            }
        ...
        case ServerOP_ZoneBootup: {
            if (pack->size != sizeof(ServerZoneStateChange_struct)) {
                std::cout << "Wrong size on ServerOP_ZoneBootup. Got: " << pack->size << ", Expected: "
                          << sizeof(ServerZoneStateChange_struct) << std::endl;
                break;
            }
            ServerZoneStateChange_struct *zst = (ServerZoneStateChange_struct *) pack->pBuffer;
            if (is_zone_loaded) {
                ...
                if (zst->zoneid == zone->GetZoneID()) {
                    ...
                    SendEmoteMessage(zst->adminname, 0, 0, "Zone bootup failed: Already running '%s'",
                                     zone->GetShortName());
                }
                break;
            }
        ...
        case ServerOP_ZoneIncClient: {
            if (pack->size != sizeof(ServerZoneIncomingClient_Struct)) {
                std::cout << "Wrong size on ServerOP_ZoneIncClient. Got: " << pack->size << ", Expected: "
                          << sizeof(ServerZoneIncomingClient_Struct) << std::endl;
                break;
            }
        ...
        case ServerOP_ZonePlayer: {
            ...
            printf("Zoning %s to %s(%u) - %u\n", client != nullptr ? client->GetCleanName() : "Unknown", szp->zone,
                   database.GetZoneID(szp->zone), szp->instance_id);
            if (client != 0) {
                if (strcasecmp(szp->adminname, szp->name) == 0)
                    ...
                    SendEmoteMessage(szp->adminname, 0, 0, "Summoning %s to %s %1.1f, %1.1f, %1.1f", szp->name,
                                     szp->zone, szp->x_pos, szp->y_pos, szp->z_pos);
                }
                if (!szp->instance_id) {
                    client->MovePC(database.GetZoneID(szp->zone), szp->instance_id, szp->x_pos, szp->y_pos, szp->z_pos,
                                   client->GetHeading(), szp->ignorerestrictions, GMSummon);
                } else {
                    if (database.GetInstanceID(client->CharacterID(), database.GetZoneID(szp->zone)) == 0) {
                        client->AssignToInstance(szp->instance_id);
                        client->MovePC(database.GetZoneID(szp->zone), szp->instance_id, szp->x_pos, szp->y_pos,
                                       szp->z_pos, client->GetHeading(), szp->ignorerestrictions, GMSummon);
                    } else {
                        client->RemoveFromInstance(
                                database.GetInstanceID(client->CharacterID(), database.GetZoneID(szp->zone)));
                        client->AssignToInstance(szp->instance_id);
                        client->MovePC(database.GetZoneID(szp->zone), szp->instance_id, szp->x_pos, szp->y_pos,
                                       szp->z_pos, client->GetHeading(), szp->ignorerestrictions, GMSummon);
                    }
        ...
        case ServerOP_KickPlayer: {
            ...
            if (client != 0) {
                if (skp->adminrank >= client->Admin()) {
                    client->WorldKick();
                    if (is_zone_loaded)
                        SendEmoteMessage(skp->adminname, 0, 0, "Remote Kick: %s booted in zone %s.", skp->name,
                                         zone->GetShortName());
                    else
                        SendEmoteMessage(skp->adminname, 0, 0, "Remote Kick: %s booted.", skp->name);
                } else if (client->GetAnon() != 1)
                    SendEmoteMessage(skp->adminname, 0, 0,
                                     "Remote Kick: Your avatar level is not high enough to kick %s", skp->name);
            }
            break;
        }
        case ServerOP_KillPlayer: {
            ...
            if (client != 0) {
                if (skp->admin >= client->Admin()) {
                    client->GMKill();
                    if (is_zone_loaded)
                        SendEmoteMessage(skp->gmname, 0, 0, "Remote Kill: %s killed in zone %s.", skp->target,
                                         zone->GetShortName());
                    else
                        SendEmoteMessage(skp->gmname, 0, 0, "Remote Kill: %s killed.", skp->target);
                } else if (client->GetAnon() != 1)
                    SendEmoteMessage(skp->gmname, 0, 0, "Remote Kill: Your avatar level is not high enough to kill %s",
                                     skp->target);
            }
            break;
        }
        ...
        case ServerOP_GMGoto: {
            if (pack->size != sizeof(ServerGMGoto_Struct)) {
                std::cout << "Wrong size on ServerOP_GMGoto. Got: " << pack->size << ", Expected: "
                          << sizeof(ServerGMGoto_Struct) << std::endl;
                break;
            }
            if (!is_zone_loaded)
                break;
            ...
            if (client != 0) {
                SendEmoteMessage(gmg->myname, 0, 13, "Summoning you to: %s @ %s, %1.1f, %1.1f, %1.1f",
                                 client->GetName(), zone->GetShortName(), client->GetX(), client->GetY(),
                                 client->GetZ());
        ...
        case ServerOP_Uptime: {
            if (pack->size != sizeof(ServerUptime_Struct)) {
                std::cout << "Wrong size on ServerOP_Uptime. Got: " << pack->size << ", Expected: "
                          << sizeof(ServerUptime_Struct) << std::endl;
                break;
            }
            ...
            uint32 s = ms / 1000;
            if (d)
                this->SendEmoteMessage(sus->adminname, 0, 0, "Zone #%i Uptime: %02id %02ih %02im %02is",
                                       sus->zoneserverid, d, h, m, s);
            else if (h)
                this->SendEmoteMessage(sus->adminname, 0, 0, "Zone #%i Uptime: %02ih %02im %02is", sus->zoneserverid, h,
                                       m, s);
            else
        ...
        case ServerOP_ChangeWID: {
            if (pack->size != sizeof(ServerChangeWID_Struct)) {
                std::cout << "Wrong size on ServerChangeWID_Struct. Got: " << pack->size << ", Expected: "
                          << sizeof(ServerChangeWID_Struct) << std::endl;
                break;
            }
        ...
        case ServerOP_Revoke: {
            ...
            if (client) {
                SendEmoteMessage(rev->adminname, 0, 0, "%s: %srevoking %s", zone->GetShortName(),
                                 rev->toggle ? "" : "un", client->GetName());
        ...
        case ServerOP_GroupInvite: {
            ...
            if (Invitee && Invitee->IsClient() && Invitee->CastToClient()->MercOnlyOrNoGroup() &&
                !Invitee->IsRaidGrouped()) {
        ...
        case ServerOP_GroupFollowAck: {
            ...
            if (groupid > 0) {
                ...
                if (group)
                    ...
                    database.SetGroupID(client->GetName(), 0, client->CharacterID(),
                                        false);    //cannot re-establish group, kill it
                }
            ...
            if (group) {
                ...
                if (!group->GetLeader()) {
                    ...
                    memset(ln, 0, 64);
                    strcpy(ln, database.GetGroupLeadershipInfo(group->GetID(), ln, MainTankName, AssistName, PullerName,
                                                               NPCMarkerName, mentoree_name, &mentor_percent, &GLAA));
                            ...
                            if (strcmp(rmsg->from, r->members[x].member->GetName()) != 0) {
                                if (r->members[x].GroupNumber == rmsg->gid) {
                                    if (r->members[x].member->GetFilter(FilterGroupChat) != 0) {
                                        r->members[x].member->ChannelMessageSend(rmsg->from,
                                                                                 r->members[x].member->GetName(), 2, 0,
                                                                                 rmsg->message);
                                    }
                                }
                            }
                    ...
                    for (int x = 0; x < MAX_RAID_MEMBERS; x++) {
                        if (r->members[x].member) {
                            if (strcmp(rmsg->from, r->members[x].member->GetName()) != 0) {
                                if (r->members[x].member->GetFilter(FilterGroupChat) != 0) {
                                    r->members[x].member->ChannelMessageSend(rmsg->from,
                                                                             r->members[x].member->GetName(), 15, 0,
                                                                             rmsg->message);
                                }
                            }
                        }
        ...
        case ServerOP_SpawnPlayerCorpse: {
            ...
            if (NewCorpse)
                ...
                Log(Logs::General, Logs::Error, "Unable to load player corpse id %u for zone %s.", s->player_corpse_id,
                    zone->GetShortName());
        ...
        case ServerOP_AdventureRequestAccept: {
            ...
            if (c) {
                c->NewAdventure(ars->id, ars->theme, ars->text, ars->member_count,
						...
						if (client_group->members[member_index] && client_group->members[member_index]->IsClient()) {
							...
							switch (update_subtype) {
								case CZLDoNUpdateSubtype_LossAdd:
									...
									break;
								case CZLDoNUpdateSubtype_LossRemove:
									client_group_member->RemoveLDoNLoss(theme_id);
									break;
								case CZLDoNUpdateSubtype_Points:
								...
								case CZLDoNUpdateSubtype_WinAdd:
									...
									break;
								case CZLDoNUpdateSubtype_WinRemove:
									client_group_member->RemoveLDoNWin(theme_id);
									break;
								default:
									break;
							}
						...
						if (client_raid_member && client_raid_member->IsClient()) {
							switch (update_subtype) {
								case CZLDoNUpdateSubtype_LossAdd:
									...
									break;
								case CZLDoNUpdateSubtype_LossRemove:
									client_raid_member->RemoveLDoNLoss(theme_id);
									break;
								case CZLDoNUpdateSubtype_Points:
								...
								case CZLDoNUpdateSubtype_WinAdd:
									...
									break;
								case CZLDoNUpdateSubtype_WinRemove:
									client_raid_member->RemoveLDoNWin(theme_id);
									break;
								default:
									break;
							}
					...
					if (client.second->GuildID() > 0 && client.second->GuildID() == update_identifier) {
						switch (update_subtype) {
							case CZLDoNUpdateSubtype_LossAdd:
								...
								break;
							case CZLDoNUpdateSubtype_LossRemove:
								client.second->RemoveLDoNLoss(theme_id);
								break;
							case CZLDoNUpdateSubtype_Points:
							...
							case CZLDoNUpdateSubtype_WinAdd:
								...
								break;
							case CZLDoNUpdateSubtype_WinRemove:
								client.second->RemoveLDoNWin(theme_id);
								break;
							default:
								break;
						}
        ...
        case ServerOP_WWMarquee: {
            ...
            while (iter != client_list.end()) {
                Client *client = (*iter);
                client->SendMarqueeMessage(WWMS->Type, WWMS->Priority, WWMS->FadeIn, WWMS->FadeOut, WWMS->Duration,
                                           Message);
                iter++;
            }
        }
